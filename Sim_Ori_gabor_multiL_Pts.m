% simulation code for Figs. 10E,F, multi-layer networks with long-range 
% tuning similarity dependent connections. 
% P_ts =0.2; muI=0, 1.2 
...
Type=strrep(sprintf('L%d_sigmaRF0d05_sigmaRR0d1_Pts%.03g',nLayer,P_ts(1)),'.','d'),
W_fname=[data_folder 'weight_', Type], 
...
if Nth>=3
    filename=strrep(sprintf('%sRF2D3layer_L%d_GaborTheta_th_sigma_n%.03g_muI%.03g_Pts%.03g_dt%.03g_ID%.0f',...
        data_folder,nLayer,sigma_n,muI,P_ts(1),dt,trial),'.','d'),
else
    filename=strrep(sprintf('%sRF2D3layer_L%d_GaborTheta_Tseg_sigma_n%.03g_muI%.03g_Pts%.03g_dt%.03g_ID%.0f',...
        data_folder,nLayer,sigma_n,muI,P_ts(1),dt,trial),'.','d'), 
end
...
if nLayer>2  % for higher-order layers, load input spike trains from lower layer
    opt.loadS1=1;
    if Nth>=3
        fname_prevL=strrep(sprintf('%sRF2D3layer_L%d_GaborTheta_th_sigma_n%.03g_muI%.03g_Pts%.03g_dt%.03g_ID%.0f',...
            data_folder,nLayer-1,sigma_n,muI,P_ts(1),dt,trial),'.','d'),
    else
        fname_prevL=strrep(sprintf('%sRF2D3layer_L%d_GaborTheta_Tseg_sigma_n%.03g_muI%.03g_Pts%.03g_dt%.03g_ID%.0f',...
            data_folder,nLayer-1,sigma_n,muI,P_ts(1),dt,trial),'.','d'),
    end
...
% save([data_folder 'theta_map'],'theta_map')
load([data_folder2 'theta_map'],'theta_map')
dx=0.04;
x=-.48:dx:.48;
[X,Y]=meshgrid(x,x);
X=X(:);Y=Y(:); 
...
if opt.useWfile==1
    if exist([W_fname '.mat'], 'file')==0
        disp('generating weight matrices')
        weight_multiL; 
    end
end
